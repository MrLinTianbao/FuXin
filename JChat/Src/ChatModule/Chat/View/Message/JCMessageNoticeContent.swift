...
open class JCMessageNoticeContent: NSObject, JCMessageContentType {
    ...
    public init(text: String,isRed:Bool?=false,money:String?="") {
        self.text = text
        self.isRed = isRed!
        self.money = money!
        super.init()
    ...
    open func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        var size = CGSize(width: mattrSize.size.width + 11, height: mattrSize.size.height + 4)
        
        if isRed {
            size = CGSize(width: mattrSize.size.width + 30, height: mattrSize.size.height + 10)
        }
