...
class JCUpdateMemberViewController: CTViewController {
    ...
    fileprivate lazy var toolView: UIView = UIView(frame: CGRect(x: 0, y: self.topOffset, width: self.view.width, height: 70))
    ...
    private func _init() {
        ...
        tableView.frame = CGRect(x: 0, y: 50 + topOffset, width: view.width, height: view.height - 50 - topOffset)
        ...
        searchBar.frame = CGRect(x: 15, y: 0, width: toolView.width - 30, height: 50)
...
//
//            })
            let groupInfo = JMSGGroupInfo.init()
            groupInfo.groupType = .public
            JMSGGroup.createGroup(with: groupInfo, memberArray: userNames) { (result, error) in
                MBProgressHUD_JChat.hide(forView: self.view, animated: true)
                if error == nil {
