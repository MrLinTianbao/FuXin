...
class GroupAvatorViewController: CTViewController {
...
//        if MJManager.isInsideService() {
            //mj群头像
            let navButton = UIButton(frame: CGRect(x: ScreenW - 45, y: StatusBarHeight, width: 35, height: NAVBarHeight))
...
//        }
        ...
        let backBtn = UIButton(frame: CGRect(x: 5, y: StatusBarHeight, width: 35, height: NAVBarHeight))
        backBtn.setImage(UIImage.init(named: "white_back"), for: .normal)
        backBtn.addTarget(self, action: #selector(_backClick), for: .touchUpInside)
        self.view.addSubview(backBtn)
    }
    @objc func _backClick() {
      self.navigationController?.popViewController()
    }
    @objc func _more() {
        
        let alertController = UIAlertController(title: nil, message: nil, preferredStyle: .actionSheet)
        let action = UIAlertAction(title: "从相册中选择", style: .default, handler: { action in
        ...
        let a2ction = UIAlertAction(title: "拍照", style: .default, handler: { action in
        ...
        alertController.addAction(a2ction)
