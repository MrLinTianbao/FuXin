...
class JCMyInfoViewController: UIViewController {
    ...
    fileprivate lazy var datePickerPopupView: UIView = {
        let popupView = UIView(frame: CGRect(x: 0, y: self.view.height - 256, width: self.view.width, height: 256))
...
//        popupView?.clickBlankSpaceDismiss = true
    ...
    fileprivate lazy var areaPickerPopupView: UIView = {
        let popupView = UIView(frame: CGRect(x: 0, y: self.view.height - 256, width: self.view.width, height: 256))
