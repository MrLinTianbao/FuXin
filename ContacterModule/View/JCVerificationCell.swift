...
import UIKit
import JMessage
protocol JCVerificationCellDelegate: class {
...
class JCVerificationCell: JCTableViewCell {
    ...
    var isGrouJoin:Bool = false
    var groupInfo: JCGroupJoinInfoModel!
    ...
    func bindDataGroup(_ info: JCGroupJoinInfoModel) {
        ...
        JMSGUser.userInfoArray(withUsernameArray: [info.username]) { (result, error) in
            if error == nil {
                ...
                self.iconView.image = UIImage.loadImage("com_icon_user_50")
            }
        }
        JMSGGroup.groupInfo(withGroupId: info.groupID) { (result, error) in
            if error == nil {
                guard let group = result as? JMSGGroup else {
                    return
                }
                self.reason.text = "申请加入 " + (group.name ?? "")
            }
        }
    ...
    var info: JCVerificationInfo!
     ...
     lazy var nickname: UILabel = UILabel()
     lazy var reason: UILabel = UILabel()
     lazy var tipInfo: UILabel = UILabel()
...
//        acceptButton.setBackgroundImage(bgImage, for: .normal)
        acceptButton.backgroundColor = UIColor.KTheme.deepOrange
    ...
    @objc func _clickAcceptButton() {
        ...
        if isGrouJoin {
            self.delegate?.clickAcceptButtonAt(info: groupInfo)
